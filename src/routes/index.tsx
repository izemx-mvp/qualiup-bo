  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
      <Panel title="À valider maintenant" description="Chaque proposition sensible attend une décision humaine.">
        <div className="space-y-3">{store.actions.filter((a) => a.status === "en attente").slice(0,4).map((a) => <div key={a.id} className="group flex flex-wrap items-center gap-4 rounded-lg border bg-surface/70 p-4 transition-all hover:-translate-y-0.5 hover:shadow-panel"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-accent/20"><Bot className="h-5 w-5 text-accent-foreground" /></div><div className="min-w-0 flex-1"><div className="font-semibold">{a.title}</div><div className="text-xs text-muted-foreground">{a.proposal.join(" · ")}</div></div><div className="text-right"><div className="num text-sm font-bold text-primary">{a.confidence}%</div><span className="text-[10px] text-muted-foreground">confiance IA</span></div><Button asChild size="sm" variant="outline"><Link to="/demandes">Examiner</Link></Button></div>)}</div>
      </Panel>
